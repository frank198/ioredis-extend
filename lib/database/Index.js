...
class Index
{
	...
	async drop(collectionName)
    {
		...
		const primary = this.schema.primaryKey(name),
			sequences = this.schema.sequencesValue(name),
			indices = this.schema.uniqueIndicesValue(name);
		...
		try
        {
			...
			this.schema.resetSchema(collectionName);
		}
		catch (err)
        {
			throw err;
	...
	{
		// Find the attribute used as a primary key
		const primary = this.schema.primaryKey(collectionName);
		// Find the index key used to keep track of all the keys in this collections
		const indexKey = this.schema.indexKey(collectionName, primary);
	...
	async create(collectionName, data)
    {
	    ...
	    {
		    const indices = this.schema.uniqueIndicesValue(name);
		    indices.forEach(idx =>
		    {
			    if (idx.name === primary)
			    {
    ...
    {
		const name = collectionName.toLowerCase(),
			primary = this.schema.primaryKey(name);
	    ...
	    // 检测自增 key
	    const sequences = this.schema.sequencesValue(name);
	    let valid = true;
	    sequences.forEach(sequenceNames =>
	    {
	...
	async uniqueConstraint(collectionName, data)
    {
		...
		const indices = this.schema.uniqueIndicesValue(collectionName);
